...
model.G=Set(initialize=[1])
model.A=Set(initialize=[(1,1,3),(1,1,2),(1,2,3)])
model.N_in_A=Set(initialize=[1,2,3])
model.o=Set(initialize=[(1,1)])
model.d=Set(initialize=[(1,3)])
model.m=Set(initialize=[(1,2),(1,4),(1,5),(1,6)])
model.c=Param(model.A,initialize={(1,1,3):4,(1,1,2):1,(1,2,3):2})


model.x=Var(model.A,domain=Binary,bounds=(0,None),)

def origin_rule(model,a,i):
    val=0
    for arc in model.A:
        if arc[0]==a and arc[1]==i:
            val+=model.x[arc]
    return val==1
model.origin=Constraint(model.o,rule=origin_rule)

def destination_rule(model,a,i):
    val=0
    for arc in model.A:
        if arc[0]==a and arc[2]==i:
            val+=model.x[arc]
    return val==1
model.destination=Constraint(model.d,rule=destination_rule)

def intermediate_rule(model,a,i):
    left_val=0
    right_val=0
    for arc in model.A:
        if arc[0]==a and arc[1]==i:
            right_val+=model.x[arc]
        elif arc[0]==a and arc[2]==i:
